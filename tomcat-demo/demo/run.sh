...
echo 'mount /dev/sda/ /mnt; podman pull quay.io/dds/multi-cloud/hello-java; podman stop hello-java;  podman run --rm -d -p 8080:8080 --name hello-java -e PLATFORM="$(cat /mnt/platform)" -e CLUSTER="$(cat /mnt/cluster)" -e CLUSTER="$(cat /mnt/product)" -e VERSION="$(cat /mnt/version)" -e ENVIRONMENT="$(cat /mnt/environment)" -e HOSTNAME="$(hostname -s) -e TYPE="tomcat-vm"" quay.io/dds/multi-cloud/hello-java'
