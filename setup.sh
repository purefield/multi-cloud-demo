...
HAPROXY_LB_ROUTE=multi-cloud.kcdc.rhlabs.io
...
HELLO_INGRESS=multi-cloud.kcdc.rhlabs.io
...
sed -i "/option httpchk GET/a \ \ \ \ http-request set-header Host ${HELLO_INGRESS}" haproxy
# Replace the value with the variable `HELLO_INGRESS`
sed -i "s/<hello_lb_hostname>/${HELLO_INGRESS}/g" haproxy
